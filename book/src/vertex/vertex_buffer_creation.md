...
显卡可以提供不同类型的内存供分配。每种类型的内存在允许的操作和性能特性方面各不相同。我们需要将缓冲区的需求和我们自己的应用程序需求结合起来，找到合适的内存类型。为此，我们创建一个新函数 `get_memory_type_index`。
...
内存分配现在就是简单地指定大小和类型，这两者都来自于顶点缓冲区的内存需求和所需的属性。创建一个 `AppData` 字段来存储内存的句柄：
